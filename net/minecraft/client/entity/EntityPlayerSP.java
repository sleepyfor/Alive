...
public class EntityPlayerSP extends AbstractClientPlayer {
    ...
    public void moveEntity(double x, double y, double z) {
        ...
        Client.INSTANCE.getEventBus().call(movementEvent);
        super.moveEntity(movementEvent.getX(), movementEvent.getY(), movementEvent.getZ());
    }
    ...
    public void onUpdate() {
        if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ))) {
            ...
            if (this.isRiding()) {
                ...
                PlayerUpdateEvent playerUpdateEvent = new PlayerUpdateEvent(rotationYaw, rotationPitch, lastReportedYaw, lastReportedPitch, posX,
                        posY, posZ, lastReportedPosX, lastReportedPosY, lastReportedPosZ, EventState.UPDATE, onGround);
                Client.INSTANCE.getEventBus().call(playerUpdateEvent);
    ...
    public void onUpdateWalkingPlayer() {
        PlayerUpdateEvent playerUpdateEvent = new PlayerUpdateEvent(rotationYaw, rotationPitch, lastReportedYaw, lastReportedPitch, posX,
                posY, posZ, lastReportedPosX, lastReportedPosY, lastReportedPosZ, EventState.PRE, onGround);
        Client.INSTANCE.getEventBus().call(playerUpdateEvent);

        SprintEvent sprintEvent = new SprintEvent(isSprinting());
        Client.INSTANCE.getEventBus().call(sprintEvent);
        boolean flag = sprintEvent.isSprint();
        ...
        if (this.isCurrentViewEntity()) {
            var eventX = playerUpdateEvent.getX();
            var eventY = playerUpdateEvent.getY();
            var eventZ = playerUpdateEvent.getZ();
            var eventLastX = playerUpdateEvent.getPrevX();
            var eventLastY = playerUpdateEvent.getPrevX();
            var eventLastZ = playerUpdateEvent.getPrevX();
            var eventYaw = playerUpdateEvent.getYaw();
            var eventPitch = playerUpdateEvent.getPitch();
            var eventLastYaw = playerUpdateEvent.getPrevYaw();
            var eventLastPitch = playerUpdateEvent.getPrevPitch();
            var eventGround = playerUpdateEvent.isGround();
            double d0 = eventX - eventLastX;
            double d1 = eventY - eventLastY;
            double d2 = eventZ - eventLastZ;
            double d3 = eventYaw - eventLastYaw;
            double d4 = eventPitch - eventLastPitch;
            ...
            playerUpdateEvent.setState(EventState.POST);
            Client.INSTANCE.getEventBus().call(playerUpdateEvent);
        }
    }
